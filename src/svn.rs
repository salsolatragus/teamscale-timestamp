...
impl<'a> Svn<'a> {
        ...
        return match opt_date_string {
            Some(ref date_string) => {
                ...
                let timestamp = opt_date_string
                    .and_then(|date_string| Svn::svn_date_string_to_timestamp(&date_string));
    ...
    fn extract_branch_from_url(url: &str) -> Option<String> {
        let regex =
            Regex::new("/(branches|tags)/(?P<branch>[^/]+)|/(?P<trunk>trunk)(/|$)").unwrap();
    ...
    pub fn branch_from_environment(&self) -> Option<String> {
        ...
        return environment_result
            .ok()
            .and_then(|url| Svn::extract_branch_from_url(&url));
    }
...
mod tests {
    ...
    fn test_extract_branch_from_url() {
        assert_eq!(
            ...
            Svn::extract_branch_from_url("https://svn.com/repo/trunk/something")
        );
        assert_eq!(
            Some("trunk".to_string()),
            Svn::extract_branch_from_url("https://svn.com/repo/trunk")
        );
        assert_eq!(
            Some("foo".to_string()),
            Svn::extract_branch_from_url("https://svn.com/repo/branches/foo/something")
        );
        assert_eq!(
            Some("bar".to_string()),
            Svn::extract_branch_from_url("https://svn.com/repo/tags/bar/")
        );
        assert_eq!(
            None,
            Svn::extract_branch_from_url("https://svn.com/repo/unrelated")
        );
        assert_eq!(
            None,
            Svn::extract_branch_from_url("https://svn.com/repo/trunkate")
        );
        assert_eq!(
            None,
            Svn::extract_branch_from_url("https://svn.com/repo/branching/blue")
        );
    }
    ...
    fn test_date_parsing() {
        assert_eq!(
            Some(1565100814),
            Svn::svn_date_string_to_timestamp("2019-08-06T14:13:34.879966Z")
        );
    }
}
